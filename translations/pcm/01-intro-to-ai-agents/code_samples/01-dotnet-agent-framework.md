...
Dis example dey show how to build intelligent travel planning agent usin Microsoft Agent Framework for .NET. Di agent fit automatic generate personalized day-trip itineraries for random destinations all over di world.
...
- 🛠️ **Custom Tool Integration**: E show how to extend agent capabilities
...
- **Azure OpenAI (Responses API)**: E dey use Azure OpenAI Responses API for model inference
- **Azure Identity**: Secure sign-in via `AzureCliCredential` (`az login`)
- **Secure Configuration**: Environment-based endpoint management

### Key Components

1. **AIAgent**: Di main agent orchestrator wey dey handle conversation flow
2. **Custom Tools**: `GetRandomDestination()` function wey the agent fit use
...
2. **Tool Integration**: Functions wey get `[Description]` attributes dey become available tools for di agent
