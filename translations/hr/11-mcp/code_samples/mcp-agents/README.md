...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-21T13:40:27+00:00",
...
Kako sve više programera istražuje alate s agentnim ponašanjem (dugotrajno izvršavanje, potreba za dodatnim unosom tijekom izvršavanja itd.), česta je zabluda da MCP nije prikladan, uglavnom zato što su se rani primjeri njegovih alata fokusirali na jednostavne obrasce zahtjev-odgovor.

Ova percepcija je zastarjela. MCP specifikacija značajno je poboljšana tijekom posljednjih nekoliko mjeseci s mogućnostima koje omogućuju izgradnju dugotrajnih agentnih ponašanja:

- **Streaming i djelomični rezultati**: Ažuriranja napretka u stvarnom vremenu tijekom izvršavanja
- **Nastavljivost**: Klijenti se mogu ponovno povezati i nastaviti nakon prekida
...
| Značajka                  | Primjer upotrebe                                                                                                                                                                       | Podrška MCP-a                                                                              |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
...
| Djelomični rezultati      | Zadatak "Generiraj knjigu" streama djelomične rezultate, npr. 1) Obrisi priče, 2) Popis poglavlja, 3) Svako poglavlje kako je dovršeno. Host može pregledati, otkazati ili preusmjeriti u bilo kojoj fazi. | ✅ Obavijesti se mogu "proširiti" kako bi uključile djelomične rezultate, vidi prijedloge na PR 383, 776 |
...
- Ponovno povezivanje nakon prekida (klijent)
- Nastavak od točke prekida (ponovno slanje poruka)

**Podrška MCP-a**: MCP StreamableHTTP transport danas podržava nastavak sesije i ponovno slanje poruka s ID-ovima sesije i ID-ovima posljednjih događaja. Važno je napomenuti da poslužitelj mora implementirati EventStore koji omogućuje ponovno slanje događaja pri ponovnom povezivanju klijenta.  
Napomena: postoji prijedlog zajednice (PR #975) koji istražuje transportno-agnostične nastavljive tokove.

| Značajka      | Primjer upotrebe                                                                                                                                                   | Podrška MCP-a                                                                |
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------- |
| Nastavljivost | Klijent se prekida tijekom dugotrajnog zadatka. Pri ponovnom povezivanju, sesija se nastavlja s propuštenim događajima koji se ponovno šalju, nastavljajući besprijekorno od točke prekida. | ✅ StreamableHTTP transport s ID-ovima sesije, ponovnim slanjem događaja i EventStore |
...
**Podrška MCP-a**: MCP sada podržava povratni tip Resource link za pozive alata. Danas je mogući obrazac dizajnirati alat koji stvara resurs i odmah vraća poveznicu na resurs. Alat može nastaviti rješavati zadatak u pozadini i ažurirati resurs. Klijent može odlučiti povremeno provjeravati stanje tog resursa kako bi dobio djelomične ili potpune rezultate (na temelju ažuriranja resursa koje poslužitelj pruža) ili se pretplatiti na resurs za obavijesti o ažuriranju.
...
| Značajka    | Primjer upotrebe                                                                                                                                        | Podrška MCP-a                                                        |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| Trajnost    | Poslužitelj se ruši tijekom zadatka migracije podataka. Rezultati i napredak preživljavaju ponovno pokretanje, klijent može provjeriti status i nastaviti iz trajnog resursa. | ✅ Poveznice na resurse s trajnim pohranama i obavijestima o statusu |
...
Specifično, implementiramo poslužitelj s dva primarna alata agenta:
...
Sljedeći odjeljci prikazuju implementaciju na strani poslužitelja za agente i rukovanje na strani klijenta za svaku značajku:
...
**Implementacija spremišta događaja (poslužitelj drži stanje sesije):**
...
- **`server/server.py`** - MCP poslužitelj s mogućnošću nastavka sa agentima za putovanja i istraživanje koji demonstriraju elicitation, sampling i ažuriranja napretka
...
- **Koordinacija više poslužitelja**: Host održava veze s više MCP poslužitelja, od kojih svaki izlaže različite mogućnosti agenata
...
Host evoluira iz jednostavnog klijenta u inteligentnog orkestratora, koordinirajući distribuirane mogućnosti agenata dok održava istu MCP protokolsku osnovu.
...
- **Eksperimentirajte**: Počnite s jednostavnim dugotrajnim zadatkom i postupno dodajte streaming, nastavljivost
