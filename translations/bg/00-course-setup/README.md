...
![Forked Repo](../../../translated_images/bg/forked-repo.33f27ca1901baa6a.png)
...
1. Изберете опцията `Fine-grained tokens` от лявата страна на екрана, като отидете в **Developer settings**.
   ...
   ![Developer settings](../../../translated_images/bg/profile_developer_settings.410a859fe749c755.png)

   След това изберете `Generate new token`.

   ![Generate Token](../../../translated_images/bg/fga_new_token.1c1a234afe202ab3.png)
...
2. Въведете описателно име за вашия токен, което отразява неговата цел, за да бъде лесно разпознаваем по-късно.
    ...
    ![Token Name and Expiration](../../../translated_images/bg/token-name-expiry-date.a095fb0de6386864.png)

3. Ограничете обхвата на токена до вашето копие на това хранилище.

    ![Limit scope to fork repository](../../../translated_images/bg/token_repository_limit.924ade5e11d9d8bb.png)
...
4. Ограничете разрешенията на токена: Под **Permissions**, кликнете върху таба **Account** и натиснете бутона "+ Add permissions". Ще се появи падащо меню. Моля, потърсете **Models** и отметнете полето за него.
    ...
    ![Add Models Permission](../../../translated_images/bg/add_models_permissions.c0c44ed8b40fc143.png)

5. Проверете необходимите разрешения преди да генерирате токена. ![Verify Permissions](../../../translated_images/bg/verify_permissions.06bd9e43987a8b21.png)

6. Преди да генерирате токена, уверете се, че сте готови да го съхраните на сигурно място, като например сейф за пароли, тъй като той няма да бъде показан отново след създаването му. ![Store Token Securely](../../../translated_images/bg/store_token_securely.08ee2274c6ad6caf.png)
...
![GitHub Token Field](../../../translated_images/bg/github_token_field.20491ed3224b5f4a.png)
...
![Project Connection String](../../../translated_images/bg/project-endpoint.8cf04c9975bbfbf1.png)
