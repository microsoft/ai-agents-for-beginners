...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-24T08:29:40+00:00",
...
> TL;DR - Maaari Ka Bang Gumawa ng Komunikasyon sa Pagitan ng Mga Ahente gamit ang MCP? Oo!
...
## Ang Maling Akala Tungkol sa Ahente/Tool
...
Lipas na ang pananaw na ito. Ang MCP specification ay malaki na ang inunlad nitong mga nakaraang buwan na may mga kakayahang nagtatanggal ng agwat para sa paggawa ng mga long-running agentic behavior:
...
> Ang isang ahente ay ating itutukoy bilang isang entity na maaaring gumana nang autonomously sa mahabang panahon, may kakayahang humawak ng mga komplikadong gawain na maaaring mangailangan ng maraming interaksyon o pagsasaayos batay sa real-time na feedback.
...
**Suporta ng MCP**: Ang mga resource update notification ay nagbibigay-daan sa streaming ng partial results, bagama't nangangailangan ito ng maingat na disenyo upang maiwasan ang mga salungatan sa 1:1 request/response model ng JSON-RPC.

| Tampok                     | Gamit                                                                                                                                                                         | Suporta ng MCP                                                                            |
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| Real-time Progress Updates | Humihiling ang user ng isang codebase migration task. Ang ahente ay nag-stream ng progreso: "10% - Sinusuri ang dependencies... 25% - Kinukonvert ang TypeScript files... 50% - Ina-update ang imports..." | ✅ Progress notifications                                                                |
| Partial Results            | Ang task na "Generate a book" ay nag-stream ng partial results, hal., 1) Balangkas ng kwento, 2) Listahan ng mga kabanata, 3) Bawat kabanata habang natatapos. Maaaring suriin, kanselahin, o i-redirect ng host sa anumang yugto. | ✅ Ang mga notification ay maaaring "palawakin" upang isama ang partial results, tingnan ang mga panukala sa PR 383, 776 |

<div align="center" style="font-style: italic; font-size: 0.95em; margin-bottom: 0.5em;">
<strong>Figure 1:</strong> Ang diagram na ito ay nagpapakita kung paano nag-stream ang isang MCP agent ng mga real-time progress updates at partial results sa host application habang nasa long-running task, na nagbibigay-daan sa user na subaybayan ang proseso sa real time.
</div>
...
Kailangang hawakan ng mga ahente ang mga network interruptions nang maayos:

- Mag-reconnect pagkatapos ng (client) disconnection
- Magpatuloy mula sa huling estado (message redelivery)

**Suporta ng MCP**: Ang MCP StreamableHTTP transport ay sumusuporta ngayon sa session resumption at message redelivery gamit ang session IDs at last event IDs. Ang mahalagang tandaan dito ay kailangang magpatupad ang server ng isang EventStore na nagbibigay-daan sa event replays kapag nag-reconnect ang client.  
Tandaan na may isang panukala mula sa komunidad (PR #975) na nag-eeksplora ng transport-agnostic resumable streams.

| Tampok       | Gamit                                                                                                                                                     | Suporta ng MCP                                                            |
| ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- |
| Resumability | Ang client ay nadidisconnect habang nasa long-running task. Kapag nag-reconnect, ang session ay nagpapatuloy na may replay ng mga na-miss na event, na nagpapatuloy nang walang putol mula sa huling estado. | ✅ StreamableHTTP transport na may session IDs, event replay, at EventStore |

<div align="center" style="font-style: italic; font-size: 0.95em; margin-bottom: 0.5em;">
<strong>Figure 2:</strong> Ang diagram na ito ay nagpapakita kung paano pinapagana ng StreamableHTTP transport at event store ng MCP ang seamless session resumption: kung madidisconnect ang client, maaari itong mag-reconnect at i-replay ang mga na-miss na event, na nagpapatuloy sa task nang walang pagkawala ng progreso.
</div>
...
**Suporta ng MCP**: Sinusuportahan na ngayon ng MCP ang isang Resource link return type para sa mga tool calls. Sa kasalukuyan, isang posibleng pattern ay ang magdisenyo ng isang tool na lumilikha ng resource at agad na nagbabalik ng resource link. Ang tool ay maaaring magpatuloy sa pagproseso ng task sa background at i-update ang resource. Sa kabilang banda, maaaring piliin ng client na i-poll ang estado ng resource upang makuha ang partial o full results (batay sa kung anong mga resource updates ang ibinibigay ng server) o mag-subscribe sa resource para sa mga update notification.

Isang limitasyon dito ay ang pag-poll ng mga resource o pag-subscribe para sa mga update ay maaaring gumamit ng mga resource na may implikasyon sa scale. May isang bukas na panukala mula sa komunidad (kabilang ang #992) na nag-eeksplora ng posibilidad ng pagsasama ng mga webhooks o triggers na maaaring tawagan ng server upang abisuhan ang client/host application ng mga update.
...
Sa kasalukuyan, isang karaniwang pattern ay ang magdisenyo ng isang tool na lumilikha ng resource at agad na nagbabalik ng resource link. Ang tool ay maaaring magpatuloy sa background sa pagproseso ng task, maglabas ng mga resource notification na nagsisilbing progress updates o naglalaman ng partial results, at i-update ang nilalaman sa resource kung kinakailangan.

<div align="center" style="font-style: italic; font-size: 0.95em; margin-bottom: 0.5em;">
<strong>Figure 3:</strong> Ang diagram na ito ay nagpapakita kung paano ginagamit ng mga MCP agents ang mga persistent resources at status notifications upang matiyak na ang mga long-running tasks ay nananatili kahit mag-restart ang server, na nagbibigay-daan sa mga client na suriin ang progreso at makuha ang mga resulta kahit pagkatapos ng mga pagkabigo.
</div>
...
**Suporta ng MCP**: Ganap na sinusuportahan sa pamamagitan ng sampling (para sa AI input) at elicitation (para sa human input).

| Tampok                 | Gamit                                                                                                                                     | Suporta ng MCP                                           |
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------- |
| Multi-Turn Interactions | Ang travel booking agent ay humihiling ng kumpirmasyon ng presyo mula sa user, pagkatapos ay humihiling sa AI na ibuod ang travel data bago tapusin ang booking transaction. | ✅ Elicitation para sa human input, sampling para sa AI input |

<div align="center" style="font-style: italic; font-size: 0.95em; margin-bottom: 0.5em;">
<strong>Figure 4:</strong> Ang diagram na ito ay nagpapakita kung paano maaaring mag-interact ang mga MCP agents upang humiling ng human input o AI assistance habang nasa proseso, na sumusuporta sa mga komplikado at multi-turn na workflows tulad ng mga kumpirmasyon at dynamic na paggawa ng desisyon.
</div>
...
Ipinapakita ng parehong ahente ang mga real-time progress updates, interactive confirmations, at buong session resumption capabilities.
...
Pinapagana ng streaming ang mga ahente na magbigay ng real-time progress updates habang nasa long-running tasks, na nagpapanatili sa mga user na may kaalaman sa task status at intermediate results.
...
#### Elicitation - Paghingi ng Input ng User

Pinapagana ng elicitation ang mga ahente na humiling ng input ng user habang nasa proseso. Mahalaga ito para sa mga kumpirmasyon, paglilinaw, o pag-apruba sa mga long-running tasks.
...
#### Sampling - Paghingi ng Tulong ng AI

Pinapagana ng sampling ang mga ahente na humiling ng tulong mula sa LLM para sa mga komplikadong desisyon o content generation habang nasa proseso. Pinapagana nito ang hybrid na human-AI workflows.

**Server Implementation (humihiling ng tulong ng AI ang ahente):**
...
#### Resumability - Pagpapatuloy ng Session sa Kabila ng Mga Pagkakakonekta

Tinitiyak ng resumability na ang mga long-running agent tasks ay maaaring magpatuloy kahit madisconnect ang client at magpatuloy nang walang putol kapag nag-reconnect. Ito ay ipinatutupad sa pamamagitan ng event stores at resumption tokens.

**Event Store Implementation (ang server ay humahawak ng session state):**
...
**Client Metadata with Resumption Token (ang client ay nagre-reconnect gamit ang naka-store na estado):**
...
Ang host application ay nagpapanatili ng session IDs at resumption tokens nang lokal, na nagbibigay-daan dito na mag-reconnect sa mga umiiral na session nang hindi nawawala ang progreso o estado.

### Organisasyon ng Code

<div align="center" style="font-style: italic; font-size: 0.95em; margin-bottom: 0.5em;">
<strong>Figure 5:</strong> Arkitektura ng sistema ng ahente na nakabatay sa MCP
</div>
...
- **`server/event_store.py`** - Event store implementation na nagpapagana ng session resumption at message redelivery

## Pagpapalawak sa Multi-Agent Communication sa MCP

Ang implementasyon sa itaas ay maaaring palawakin sa mga multi-agent systems sa pamamagitan ng pagpapahusay sa katalinuhan at saklaw ng host application:
...
- **Multi-Server Coordination**: Pinapanatili ng host ang koneksyon sa maraming MCP servers, bawat isa ay nag-eexpose ng iba't ibang kakayahan ng ahente
- **Task State Management**: Sinusubaybayan ng host ang progreso sa maraming sabay-sabay na agent tasks, hinahawakan ang mga dependencies at sequencing
- **Resilience & Retries**: Pinamamahalaan ng host ang mga pagkabigo, nagpapatupad ng retry logic, at nire-reroute ang mga task kapag naging unavailable ang mga ahente
- **Result Synthesis**: Pinagsasama-sama ng host ang mga output mula sa maraming ahente sa isang malinaw na panghuling resulta
...
### 3. Mag-eksperimento at Palawakin
