...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-21T12:55:30+00:00",
...
- **التفاعلية متعددة الأدوار**: إدخال تفاعلي أثناء التنفيذ عبر الاستخلاص وأخذ العينات

يمكن تركيب هذه الميزات لتمكين تطبيقات وكيلة معقدة ومتعددة الوكلاء، يتم نشرها جميعًا على بروتوكول MCP.

للتوضيح، سنشير إلى الوكيل كـ "أداة" متاحة على خادم MCP. وهذا يعني وجود تطبيق مضيف ينفذ عميل MCP الذي ينشئ جلسة مع خادم MCP ويمكنه استدعاء الوكيل.

## ما الذي يجعل أداة MCP "وكيلة"؟

قبل الغوص في التنفيذ، دعونا نحدد القدرات التحتية اللازمة لدعم الوكلاء الذين يعملون لفترات طويلة.
...
أنماط الطلب-الاستجابة التقليدية لا تعمل مع المهام طويلة الأمد. يحتاج الوكلاء إلى توفير:
...
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------ |
| تحديثات التقدم في الوقت الفعلي | يطلب المستخدم مهمة ترحيل قاعدة بيانات الكود. يقوم الوكيل بتدفق التقدم: "10% - تحليل التبعيات... 25% - تحويل ملفات TypeScript... 50% - تحديث الواردات..."                     | ✅ إشعارات التقدم                                                                          |
| النتائج الجزئية            | مهمة "إنشاء كتاب" تبث النتائج الجزئية، مثل: 1) مخطط القصة، 2) قائمة الفصول، 3) كل فصل عند اكتماله. يمكن للمضيف الفحص أو الإلغاء أو إعادة التوجيه في أي مرحلة. | ✅ يمكن "تمديد" الإشعارات لتشمل النتائج الجزئية، انظر المقترحات على PR 383، 776          |

<div align="center" style="font-style: italic; font-size: 0.95em; margin-bottom: 0.5em;">
<strong>الشكل 1:</strong> يوضح هذا الرسم البياني كيف يقوم وكيل MCP ببث تحديثات التقدم في الوقت الفعلي والنتائج الجزئية إلى تطبيق المضيف أثناء مهمة طويلة الأمد، مما يتيح للمستخدم مراقبة التنفيذ في الوقت الفعلي.
</div>
...
<div align="center" style="font-style: italic; font-size: 0.95em; margin-bottom: 0.5em;">
<strong>الشكل 2:</strong> يوضح هذا الرسم البياني كيف يمكن لنقل StreamableHTTP الخاص بـ MCP ومخزن الأحداث تمكين استئناف الجلسات بسلاسة: إذا انقطع العميل، يمكنه إعادة الاتصال وإعادة تشغيل الأحداث الفائتة، مما يتيح استمرار المهمة دون فقدان التقدم.
</div>
...
**دعم MCP**: يدعم MCP الآن نوع إرجاع رابط الموارد لاستدعاءات الأدوات. اليوم، يمكن تصميم نمط يقوم فيه الأداة بإنشاء مورد وإرجاع رابط المورد فورًا. يمكن للأداة متابعة معالجة المهمة في الخلفية وتحديث المورد. بدوره، يمكن للعميل اختيار استقصاء حالة هذا المورد للحصول على نتائج جزئية أو كاملة (بناءً على تحديثات الموارد التي يوفرها الخادم) أو الاشتراك في المورد للحصول على إشعارات التحديث.

أحد القيود هنا هو أن استقصاء الموارد أو الاشتراك للحصول على التحديثات يمكن أن يستهلك الموارد مع تداعيات على النطاق. هناك اقتراح مجتمعي مفتوح (بما في ذلك #992) يستكشف إمكانية تضمين webhooks أو مشغلات يمكن للخادم استدعاؤها لإخطار تطبيق العميل/المضيف بالتحديثات.
...
اليوم، النمط الشائع هو تصميم أداة تقوم بإنشاء مورد وإرجاع رابط المورد فورًا. يمكن للأداة في الخلفية معالجة المهمة، إصدار إشعارات الموارد التي تعمل كتحديثات تقدمية أو تتضمن نتائج جزئية، وتحديث المحتوى في المورد حسب الحاجة.
...
**دعم MCP**: مدعوم بالكامل عبر أخذ العينات (لإدخال الذكاء الاصطناعي) والاستخلاص (لإدخال البشر).

| الميزة                 | حالة الاستخدام                                                                                                                                     | دعم MCP                                           |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------- |
| التفاعلات متعددة الأدوار | وكيل حجز السفر يطلب تأكيد السعر من المستخدم، ثم يطلب من الذكاء الاصطناعي تلخيص بيانات السفر قبل إكمال عملية الحجز. | ✅ الاستخلاص لإدخال البشر، أخذ العينات لإدخال الذكاء الاصطناعي |

<div align="center" style="font-style: italic; font-size: 0.95em; margin-bottom: 0.5em;">
<strong>الشكل 4:</strong> يوضح هذا الرسم البياني كيف يمكن لوكلاء MCP طلب إدخال تفاعلي من البشر أو طلب مساعدة الذكاء الاصطناعي أثناء التنفيذ، مما يدعم سير العمل المعقد متعدد الأدوار مثل التأكيدات واتخاذ القرارات الديناميكية.
</div>
...
كجزء من هذا المقال، نقدم [مستودع كود](https://github.com/victordibia/ai-tutorials/tree/main/MCP%20Agents) يحتوي على تنفيذ كامل للوكلاء طويلة الأمد باستخدام MCP Python SDK مع نقل StreamableHTTP لاستئناف الجلسات وإعادة تسليم الرسائل. يوضح التنفيذ كيف يمكن تركيب قدرات MCP لتمكين سلوكيات وكيلة متطورة.

على وجه التحديد، نقوم بتنفيذ خادم يحتوي على أداتين رئيسيتين للوكيل:

- **وكيل السفر** - يحاكي خدمة حجز السفر مع تأكيد السعر عبر الاستخلاص
...
**تنفيذ العميل (المضيف يتلقى تحديثات التقدم):**
...
#### الاستخلاص - طلب إدخال المستخدم

يتيح الاستخلاص للوكلاء طلب إدخال المستخدم أثناء التنفيذ. هذا ضروري للتأكيدات أو التوضيحات أو الموافقات أثناء المهام طويلة الأمد.
...
**تنفيذ العميل (المضيف يوفر ردود الاستخلاص):**
...
يتيح أخذ العينات للوكلاء طلب مساعدة LLM لاتخاذ قرارات معقدة أو إنشاء محتوى أثناء التنفيذ. هذا يمكّن من سير العمل الهجين بين البشر والذكاء الاصطناعي.
...
**تنفيذ العميل (المضيف يوفر ردود أخذ العينات):**
...
تضمن القابلية للاستئناف أن المهام طويلة الأمد للوكيل يمكن أن تبقى مستمرة حتى بعد انقطاع العميل وتستمر بسلاسة عند إعادة الاتصال. يتم تنفيذ ذلك من خلال مخازن الأحداث ورموز الاستئناف.
...
يحافظ تطبيق المضيف على معرفات الجلسات ورموز الاستئناف محليًا، مما يتيح له إعادة الاتصال بالجلسات الحالية دون فقدان التقدم أو الحالة.
...
- **`server/server.py`** - خادم MCP قابل للاستئناف مع وكلاء السفر والبحث الذين يظهرون الاستخلاص، أخذ العينات، وتحديثات التقدم
- **`client/client.py`** - تطبيق مضيف تفاعلي مع دعم الاستئناف، معالجات ردود الاتصال، وإدارة الرموز
- **`server/event_store.py`** - تنفيذ مخزن الأحداث الذي يتيح استئناف الجلسات وإعادة تسليم الرسائل
...
- **توليف النتائج**: يجمع المضيف المخرجات من وكلاء متعددين في نتائج نهائية متماسكة

يتطور المضيف من عميل بسيط إلى منسق ذكي، ينسق قدرات الوكلاء الموزعة مع الحفاظ على نفس أساس بروتوكول MCP.

## الخاتمة

تمكن قدرات MCP المحسنة - إشعارات الموارد، الاستخلاص/أخذ العينات، التدفقات القابلة للاستئناف، والموارد المستمرة - من تفاعلات معقدة بين الوكلاء مع الحفاظ على بساطة البروتوكول.

## البدء

جاهز لبناء نظام agent2agent الخاص بك؟ اتبع هذه الخطوات:
...
- `travel_agent` - حجز السفر مع تأكيد السعر عبر الاستخلاص
