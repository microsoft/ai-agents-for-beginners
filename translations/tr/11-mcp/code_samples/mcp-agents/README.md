...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-21T13:17:55+00:00",
...
Giderek daha fazla geliştirici, agentik davranışlar sergileyen araçları (uzun süre çalışabilen, yürütme sırasında ek girdilere ihtiyaç duyabilen vb.) keşfettikçe, MCP'nin uygun olmadığına dair yaygın bir yanılgı ortaya çıkıyor. Bunun nedeni, MCP'nin araçlar için sunduğu ilk örneklerin basit istek-cevap modellerine odaklanmış olmasıdır.

Bu algı artık geçerli değil. MCP spesifikasyonu, uzun süreli agentik davranışlar oluşturma konusunda boşlukları kapatan özelliklerle son birkaç ayda önemli ölçüde geliştirildi:
...
Bu özellikler, MCP protokolü üzerinde karmaşık agentik ve çoklu agent uygulamalarını etkinleştirmek için bir araya getirilebilir.

Referans olarak, bir agent'i MCP sunucusunda mevcut olan bir "araç" olarak adlandıracağız. Bu, bir MCP istemcisini uygulayan ve MCP sunucusuyla bir oturum kurarak agent'i çağırabilen bir ana uygulamanın varlığını ima eder.
...
> Bir agent'i, uzun süre boyunca bağımsız olarak çalışabilen, birden fazla etkileşim veya gerçek zamanlı geri bildirimlere dayalı ayarlamalar gerektiren karmaşık görevleri yerine getirebilen bir varlık olarak tanımlayacağız.

### 1. Akış ve Kısmi Sonuçlar

Geleneksel istek-cevap modelleri, uzun süreli görevler için uygun değildir. Agent'lerin şunları sağlaması gerekir:

- Gerçek zamanlı ilerleme güncellemeleri
- Ara sonuçlar

**MCP Desteği**: Kaynak güncelleme bildirimleri, kısmi sonuçların akışını sağlar, ancak bu, JSON-RPC'nin 1:1 istek/cevap modelindeki çatışmaları önlemek için dikkatli bir tasarım gerektirir.

| Özellik                    | Kullanım Durumu                                                                                                                                                                       | MCP Desteği                                                                                |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| Gerçek Zamanlı İlerleme Güncellemeleri | Kullanıcı bir kod tabanı geçiş görevi talep eder. Agent ilerlemeyi akış olarak bildirir: "10% - Bağımlılıkları analiz ediyor... 25% - TypeScript dosyalarını dönüştürüyor... 50% - İthalatları güncelliyor..."          | ✅ İlerleme bildirimleri                                                                  |
| Kısmi Sonuçlar            | "Bir kitap oluştur" görevi kısmi sonuçları akış olarak bildirir, örneğin: 1) Hikaye yayının taslağı, 2) Bölüm listesi, 3) Tamamlanan her bölüm. Ana uygulama her aşamada inceleyebilir, iptal edebilir veya yönlendirebilir. | ✅ Bildirimler "genişletilebilir" ve kısmi sonuçlar içerebilir, PR 383, 776 önerilerine bakın |
...
Agent'ler ağ kesintilerini sorunsuz bir şekilde yönetebilmelidir:
...
- Durum dışarıdan alınabilir
- Oturumlar arasında ilerleme takibi

**MCP Desteği**: MCP artık araç çağrıları için bir Kaynak bağlantı dönüş türünü destekler. Bugün, bir araç tasarlamak ve hemen bir kaynak bağlantısı döndürmek yaygın bir modeldir. Araç, arka planda görevi ele alabilir ve kaynağı güncelleyebilir. Buna karşılık, istemci bu kaynağın durumunu kontrol etmeyi, kısmi veya tam sonuçları almayı (sunucunun sağladığı kaynak güncellemelerine bağlı olarak) veya güncelleme bildirimleri için kaynağa abone olmayı seçebilir.

Buradaki bir sınırlama, kaynakları kontrol etmenin veya güncellemeler için abone olmanın ölçek açısından kaynak tüketimiyle ilgili sonuçları olabilmesidir. Topluluk önerisi (örneğin #992), sunucunun istemci/ana uygulamayı güncellemeler hakkında bilgilendirmek için çağırabileceği webhook'lar veya tetikleyiciler ekleme olasılığını keşfetmektedir.
...
Bugün, bir araç tasarlamak ve hemen bir kaynak bağlantısı döndürmek yaygın bir modeldir. Araç, arka planda görevi ele alabilir, ilerleme güncellemeleri veya kısmi sonuçlar olarak hizmet eden kaynak bildirimleri yayınlayabilir ve gerektiğinde kaynaktaki içeriği güncelleyebilir.

<div align="center" style="font-style: italic; font-size: 0.95em; margin-bottom: 0.5em;">
<strong>Şekil 3:</strong> Bu diyagram, MCP agent'lerinin kalıcı kaynaklar ve durum bildirimlerini kullanarak uzun süreli görevlerin sunucu yeniden başlatmalarından sonra hayatta kalmasını nasıl sağladığını, istemcilerin ilerlemeyi kontrol etmesine ve başarısızlıklardan sonra sonuçları almasına olanak tanıdığını göstermektedir.
</div>
...
- İnsan açıklamaları veya onayları
...
<div align="center" style="font-style: italic; font-size: 0.95em; margin-bottom: 0.5em;">
<strong>Şekil 4:</strong> Bu diyagram, MCP agent'lerinin yürütme sırasında insan girdisini etkileşimli olarak toplamasını veya karmaşık, çoklu dönüş iş akışlarını desteklemek için AI yardımı istemesini, onaylar ve dinamik karar verme gibi işlemleri nasıl gerçekleştirdiğini göstermektedir.
</div>
...
Her iki agent de gerçek zamanlı ilerleme güncellemeleri, etkileşimli onaylar ve tam oturum devamlılığı yeteneklerini gösterir.
...
Akış, agent'lerin uzun süreli görevler sırasında gerçek zamanlı ilerleme güncellemeleri sağlamasına, kullanıcıları görev durumu ve ara sonuçlar hakkında bilgilendirmesine olanak tanır.
...
Bilgi toplama, agent'lerin yürütme sırasında kullanıcı girdisi istemesine olanak tanır. Bu, uzun süreli görevler sırasında onaylar, açıklamalar veya onaylar için gereklidir.
...
**İstemci Uygulaması (ana uygulama bilgi toplama geri çağrısı sağlar):**
...
Örnekleme, agent'lerin yürütme sırasında karmaşık kararlar veya içerik oluşturma için LLM yardımı istemesine olanak tanır. Bu, hibrit insan-AI iş akışlarını mümkün kılar.
...
Devam edilebilirlik, uzun süreli agent görevlerinin istemci kesintilerini hayatta kalmasını ve yeniden bağlanma sırasında sorunsuz bir şekilde devam etmesini sağlar. Bu, olay depoları ve devamlılık belirteçleri aracılığıyla uygulanır.
...
- **`server/server.py`** - Seyahat ve araştırma agent'lerini içeren, bilgi toplama, örnekleme ve ilerleme güncellemelerini gösteren devamlı MCP sunucusu
...
- **Sonuç Sentezi**: Ana uygulama, birden fazla agent'ten gelen çıktıları tutarlı nihai sonuçlara dönüştürür
...
MCP'nin geliştirilmiş yetenekleri - kaynak bildirimleri, bilgi toplama/örnekleme, devam edilebilir akışlar ve kalıcı kaynaklar - protokol basitliğini korurken karmaşık agent-to-agent etkileşimlerini mümkün kılar.
...
Bu, MCP'nin araç tabanlı basitliği kor
