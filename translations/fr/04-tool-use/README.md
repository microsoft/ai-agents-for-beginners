...
Les outils sont intéressants car ils permettent aux agents d'IA d'avoir un éventail plus large de capacités. Au lieu que l'agent dispose d'un ensemble limité d'actions qu'il peut effectuer, en ajoutant un outil, l'agent peut désormais exécuter un large éventail d'actions. Dans ce chapitre, nous allons examiner le modèle de conception d'utilisation d'outils, qui décrit comment les agents d'IA peuvent utiliser des outils spécifiques pour atteindre leurs objectifs.

## Introduction

Dans cette leçon, nous cherchons à répondre aux questions suivantes :

- Quel est le modèle de conception d'utilisation d'outils ?
...
- Quelles sont les considérations particulières pour utiliser le modèle de conception d'utilisation d'outils afin de construire des agents d'IA dignes de confiance ?
...
Le **modèle de conception d'utilisation d'outils** se concentre sur la capacité des LLM à interagir avec des outils externes pour atteindre des objectifs spécifiques. Les outils sont du code qui peut être exécuté par un agent pour effectuer des actions. Un outil peut être une fonction simple telle qu'une calculatrice, ou un appel API à un service tiers comme la consultation du cours d'une action ou les prévisions météorologiques. Dans le contexte des agents d'IA, les outils sont conçus pour être exécutés par les agents en réponse à des **appels de fonction générés par le modèle**.

## À quels cas d'utilisation peut-il s'appliquer ?

Les agents d'IA peuvent exploiter les outils pour accomplir des tâches complexes, récupérer des informations ou prendre des décisions. Le modèle d'utilisation d'outils est souvent utilisé dans des scénarios nécessitant une interaction dynamique avec des systèmes externes, tels que des bases de données, des services web ou des interprètes de code. Cette capacité est utile pour un certain nombre de cas d'utilisation différents, notamment :

- **Récupération dynamique d'informations :** Les agents peuvent interroger des API externes ou des bases de données pour obtenir des données à jour (par exemple, interroger une base de données SQLite pour l'analyse de données, récupérer les cours des actions ou les informations météorologiques).
...
- **Automatisation de flux de travail :** Automatiser des workflows répétitifs ou multi-étapes en intégrant des outils comme des planificateurs de tâches, des services de messagerie électronique ou des pipelines de données.
...
- **Génération et édition de contenu :** Les agents peuvent exploiter des outils comme les correcteurs grammaticaux, les résumeurs de texte ou les évaluateurs de sécurité de contenu pour aider aux tâches de création de contenu.
...
Ces blocs de construction permettent à l'agent d'IA d'exécuter un large éventail de tâches. Voyons les éléments clés nécessaires pour implémenter le modèle de conception d'utilisation d'outils :

- **Schémas de fonction/outils** : Définitions détaillées des outils disponibles, y compris le nom de la fonction, l'objectif, les paramètres requis et les sorties attendues. Ces schémas permettent au LLM de comprendre quels outils sont disponibles et comment construire des requêtes valides.
...
- **Système de gestion des messages** : Composants qui gèrent le flux conversationnel entre les entrées utilisateur, les réponses du LLM, les appels aux outils et les sorties des outils.
...
- **Gestion de l'état** : Suit le contexte de la conversation, les interactions précédentes avec les outils et les données persistantes pour assurer la cohérence lors des interactions multi-tours.

Ensuite, examinons l'appel de fonctions/outils plus en détail.

### Appel de fonctions/outils

L'appel de fonctions est le principal moyen par lequel nous permettons aux modèles de langage haute capacité (LLM) d'interagir avec des outils. Vous verrez souvent 'Fonction' et 'Outil' utilisés de manière interchangeable car les 'fonctions' (blocs de code réutilisables) sont les 'outils' que les agents utilisent pour accomplir des tâches. Pour qu'un code de fonction soit invoqué, un LLM doit comparer la requête de l'utilisateur avec la description des fonctions. Pour cela, un schéma contenant les descriptions de toutes les fonctions disponibles est envoyé au LLM. Le LLM sélectionne alors la fonction la plus appropriée pour la tâche et retourne son nom et ses arguments. La fonction sélectionnée est invoquée, sa réponse est renvoyée au LLM, qui utilise l'information pour répondre à la demande de l'utilisateur.
...
1. **Initialiser un LLM supportant l'appel de fonctions :**

    Tous les modèles ne supportent pas l'appel de fonctions, il est donc important de vérifier que le LLM que vous utilisez le fait. <a href="https://learn.microsoft.com/azure/ai-services/openai/how-to/function-calling" target="_blank">Azure OpenAI</a> supporte l'appel de fonctions. Nous pouvons commencer par initier le client Azure OpenAI.
...
1. **Créer un schéma de fonction :**
    ...
    Nous prendrons ensuite ce schéma et le transmettrons au client créé précédemment, ainsi que la requête de l'utilisateur pour trouver l'heure à San Francisco. Il est important de noter qu'un **appel d'outil** est ce qui est retourné, **pas** la réponse finale à la question. Comme mentionné précédemment, le LLM retourne le nom de la fonction qu'il a sélectionnée pour la tâche, et les arguments qui y seront passés.
...
L'appel de fonctions est au cœur de la plupart, sinon de la totalité, des conceptions d'utilisation d'outils par agents, cependant l'implémenter à partir de zéro peut parfois être un défi.
Comme nous l'avons appris dans la [Leçon 2](../../../02-explore-agentic-frameworks), les frameworks agentiques nous fournissent des blocs de construction préfabriqués pour implémenter l'utilisation d'outils.
...
<a href="https://learn.microsoft.com/azure/ai-services/agents/overview" target="_blank">Azure AI Agent Service</a> est un framework agentique plus récent conçu pour permettre aux développeurs de construire, déployer et mettre à l'échelle de manière sécurisée des agents IA de haute qualité et extensibles sans avoir à gérer les ressources de calcul et de stockage sous-jacentes. Il est particulièrement utile pour les applications d'entreprise car c'est un service entièrement géré avec une sécurité de niveau entreprise.

Comparé au développement directement avec l'API LLM, Azure AI Agent Service offre plusieurs avantages, notamment :

- Appel automatique d'outils – plus besoin de parser un appel d'outil, d'invoquer l'outil et de gérer la réponse ; tout cela est désormais réalisé côté serveur
- Données gérées de manière sécurisée – au lieu de gérer votre propre état de conversation, vous pouvez vous appuyer sur les fils (threads) pour stocker toutes les informations nécessaires
- Outils prêts à l'emploi – des outils que vous pouvez utiliser pour interagir avec vos sources de données, tels que Bing, Azure AI Search et Azure Functions.
...
Le service Agent nous permet d'utiliser ces outils ensemble en tant qu'`ensemble d'outils` (`toolset`). Il utilise également des `threads` qui gardent trace de l'historique des messages d'une conversation particulière.

Imaginez que vous êtes un agent commercial dans une entreprise appelée Contoso. Vous souhaitez développer un agent conversationnel capable de répondre aux questions sur vos données de ventes.
...
Pour utiliser l'un de ces outils avec le service, nous pouvons créer un client et définir un outil ou un ensemble d'outils. Pour implémenter cela pratiquement, nous pouvons utiliser le code Python suivant. Le LLM pourra regarder l'ensemble d'outils et décider s'il faut utiliser la fonction créée par l'utilisateur, `fetch_sales_data_using_sqlite_query`, ou l'interpréteur de code pré-construit selon la requête utilisateur.
...
# Initialiser l'outil d'interpréteur de code et l'ajouter à l'ensemble d'outils.
