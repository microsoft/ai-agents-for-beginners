...
![초보자를 위한 생성 AI](../../translated_images/ko/repo-thumbnailv2.06f4a48036fde647.webp)
...
[![GitHub PR](https://img.shields.io/github/issues-pr/microsoft/ai-agents-for-beginners.svg)](https://GitHub.com/microsoft/ai-agents-for-beginners/pulls/?WT.mc_id=academic-105485-koreyst)
...
**추가 번역 언어를 원하면 [여기](https://github.com/Azure/co-op-translator/blob/main/getting_started/supported-languages.md)에서 지원 언어 목록을 확인하세요**
...
이 과정은 AI 에이전트를 구축하는 기본을 다루는 수업들로 구성되어 있습니다. 각 수업은 자체 주제를 다루므로 원하는 곳에서 시작하세요!

이 과정은 다국어를 지원합니다. [이용 가능한 언어 목록](../..)을 확인하세요.

만약 생성 AI 모델로 처음 빌드하는 거라면, 21개의 생성 AI 빌드 수업이 포함된 [초보자를 위한 생성 AI](https://aka.ms/genai-beginners) 강의를 참고하세요.

이 저장소에 [별표(🌟) 달기](https://docs.github.com/en/get-started/exploring-projects-on-github/saving-repositories-with-stars?WT.mc_id=academic-105485-koreyst)와 [포크하기](https://github.com/microsoft/ai-agents-for-beginners/fork) 해서 코드를 실행하는 것도 잊지 마세요.

### 다른 학습자 만나기, 질문에 답 얻기

AI 에이전트 구축에 대해 막히거나 질문이 있으면 [Microsoft Foundry Discord](https://aka.ms/ai-agents/discord) 내 전용 Discord 채널에 참여하세요.

### 필요한 것

이 과정의 각 수업에는 code_samples 폴더에 있는 코드 예제가 포함되어 있습니다. [이 저장소를 포크](https://github.com/microsoft/ai-agents-for-beginners/fork) 해 자신의 복사본을 만드세요.

연습에서 사용하는 코드 예제는 Azure AI Foundry와 GitHub 모델 카탈로그를 이용하여 언어 모델과 상호작용합니다:
...
또한 이 과정은 Microsoft의 다음 AI 에이전트 프레임워크 및 서비스를 활용합니다:
...
코드 실행 관련 자세한 내용은 [강의 설정](./00-course-setup/README.md)을 참고하세요.

## 🙏 도움을 주시겠어요?

제안사항이나 철자 또는 코드 오류를 발견하셨나요? [이슈 제기](https://github.com/microsoft/ai-agents-for-beginners/issues?WT.mc_id=academic-105485-koreyst) 또는 [풀 리퀘스트 생성](https://github.com/microsoft/ai-agents-for-beginners/pulls?WT.mc_id=academic-105485-koreyst) 하세요.



## 📂 각 수업에는 포함됨

- README의 글 수업 및 짧은 동영상
...
| 에이전틱 프로토콜 사용법 (MCP, A2A 및 NLWeb) | [Link](./11-agentic-protocols/README.md)           | [Video](https://youtu.be/X-Dh9R3Opn8)                                 | [Link](https://aka.ms/ai-agents-beginners/collection?WT.mc_id=academic-105485-koreyst) |
...
| Microsoft 에이전트 프레임워크 탐험                         | [Link](./14-microsoft-agent-framework/README.md)                            |                                                            |                                                                                        |
...
이 프로젝트는 기여와 제안을 환영합니다. 대부분의 기여는 기여권 계약서(CLA)에 동의해야 하며, 이는 여러분이 해당 기여에 대한 권리를 가지고 실제로 권리를 부여한다는 것을 선언하는 문서입니다. 자세한 내용은 <https://cla.opensource.microsoft.com>를 참조하세요.

풀 리퀘스트를 제출하면, CLA 봇이 자동으로 CLA 제출 여부를 판단하고 PR에 적절한 표시(예: 상태 검사, 댓글)를 붙입니다. 봇이 제공하는 지침을 따르면 됩니다. 이 과정은 모든 레포지토리에서 한 번만 수행하면 됩니다.

이 프로젝트는 [Microsoft 오픈 소스 행동 강령](https://opensource.microsoft.com/codeofconduct/)을 채택했습니다.
자세한 내용은 [행동 강령 FAQ](https://opensource.microsoft.com/codeofconduct/faq/)를 참고하거나 추가 질문이나 의견이 있으면 [opencode@microsoft.com](mailto:opencode@microsoft.com)으로 문의하세요.
...
이 프로젝트를 수정한 버전에서 Microsoft 상표 또는 로고를 사용하는 경우 혼동을 일으키거나 Microsoft 후원을 암시해서는 안 됩니다.
...
**면책 조항**:  
이 문서는 AI 번역 서비스 [Co-op Translator](https://github.com/Azure/co-op-translator)를 사용하여 번역되었습니다. 우리는 정확성을 위해 노력하고 있으나, 자동 번역에는 오류나 부정확한 부분이 있을 수 있음을 알려드립니다. 원본 문서의 원어 버전이 권위 있는 출처로 간주되어야 합니다. 중요한 정보의 경우 전문적인 인간 번역을 권장합니다. 본 번역의 사용으로 인해 발생하는 오해나 오역에 대해 당사는 책임을 지지 않습니다.
