...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-30T07:40:55+00:00",
...
وظایف بزرگ یا پیچیده با تقسیم به وظایف فرعی کوچک‌تر و هدف‌محور قابل مدیریت‌تر می‌شوند.
...
هر وظیفه فرعی سپس می‌تواند توسط عوامل یا فرآیندهای اختصاصی انجام شود. یک عامل ممکن است در جستجوی بهترین پیشنهادات پرواز تخصص داشته باشد، دیگری بر رزرو هتل تمرکز کند، و غیره. یک عامل هماهنگ‌کننده یا "پایین‌دستی" سپس می‌تواند این نتایج را به یک برنامه منسجم برای کاربر نهایی ترکیب کند.
...
مدل‌های زبان بزرگ (LLMs) می‌توانند خروجی ساختاریافته (مانند JSON) تولید کنند که برای عوامل یا خدمات پایین‌دستی آسان‌تر قابل تجزیه و پردازش است. این امر به ویژه در زمینه چندعاملی مفید است، جایی که می‌توان این وظایف را پس از دریافت خروجی برنامه‌ریزی انجام داد. برای یک مرور سریع به نمونه زیر توجه کنید.
...
خروجی کد قبلی به شرح زیر است و سپس می‌توانید از این خروجی ساختاریافته برای ارسال به `assigned_agent` و خلاصه‌سازی برنامه سفر برای کاربر نهایی استفاده کنید.
...
یک نوت‌بوک نمونه با کد قبلی [اینجا](07-autogen.ipynb) موجود است.

### برنامه‌ریزی تکراری

برخی وظایف نیاز به رفت و برگشت یا برنامه‌ریزی مجدد دارند، جایی که نتیجه یک وظیفه فرعی بر وظیفه بعدی تأثیر می‌گذارد. به عنوان مثال، اگر عامل هنگام رزرو پروازها یک فرمت داده غیرمنتظره کشف کند، ممکن است نیاز به تطبیق استراتژی خود قبل از ادامه رزرو هتل داشته باشد.
