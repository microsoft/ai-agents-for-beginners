...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-24T09:25:37+00:00",
...
- **قابلیت از سرگیری**: مشتریان می‌توانند پس از قطع ارتباط دوباره متصل شوند و ادامه دهند
- **دوام**: نتایج پس از راه‌اندازی مجدد سرور باقی می‌مانند (مثلاً از طریق لینک‌های منابع)
- **چند نوبتی**: ورودی تعاملی در میانه اجرا از طریق الیسیتیشن و نمونه‌گیری

این ویژگی‌ها می‌توانند ترکیب شوند تا برنامه‌های پیچیده عامل‌گونه و چندعاملی را فعال کنند که همگی بر اساس پروتکل MCP مستقر شده‌اند.
...
<strong>شکل 1:</strong> این نمودار نشان می‌دهد که چگونه یک عامل MCP به‌روزرسانی‌های پیشرفت در زمان واقعی و نتایج جزئی را به برنامه میزبان جریان می‌دهد، و به کاربر امکان می‌دهد اجرای وظیفه را در زمان واقعی نظارت کند.
</div>
...
عوامل باید به طور مؤثر با وقفه‌های شبکه کنار بیایند:

- اتصال مجدد پس از قطع ارتباط (مشتری)
- ادامه از جایی که متوقف شده بودند (بازپخش پیام‌ها)

**پشتیبانی MCP**: انتقال StreamableHTTP MCP امروز از از سرگیری جلسه و بازپخش پیام‌ها با شناسه‌های جلسه و شناسه‌های آخرین رویداد پشتیبانی می‌کند. نکته مهم این است که سرور باید یک EventStore پیاده‌سازی کند که امکان بازپخش رویدادها را در اتصال مجدد مشتری فراهم کند.  
توجه داشته باشید که یک پیشنهاد جامعه (PR #975) وجود دارد که جریان‌های قابل از سرگیری مستقل از انتقال را بررسی می‌کند.

| ویژگی      | مورد استفاده                                                                                                                                                   | پشتیبانی MCP                                                                |
| ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- |
| قابلیت از سرگیری | مشتری در طول یک وظیفه طولانی‌مدت قطع می‌شود. پس از اتصال مجدد، جلسه با رویدادهای از دست رفته بازپخش شده و بدون وقفه از جایی که متوقف شده بود ادامه می‌یابد. | ✅ انتقال StreamableHTTP با شناسه‌های جلسه، بازپخش رویداد و EventStore |

<div align="center" style="font-style: italic; font-size: 0.95em; margin-bottom: 0.5em;">
<strong>شکل 2:</strong> این نمودار نشان می‌دهد که چگونه انتقال StreamableHTTP و EventStore MCP امکان از سرگیری بدون وقفه جلسه را فراهم می‌کنند: اگر مشتری قطع شود، می‌تواند دوباره متصل شود و رویدادهای از دست رفته را بازپخش کند و وظیفه را بدون از دست دادن پیشرفت ادامه دهد.
</div>
...
یکی از محدودیت‌ها این است که بررسی منابع یا اشتراک برای به‌روزرسانی‌ها می‌تواند منابع را مصرف کند و در مقیاس تأثیراتی داشته باشد. یک پیشنهاد جامعه باز (شامل #992) امکان گنجاندن وب‌هوک‌ها یا تریگرهایی که سرور می‌تواند برای اطلاع‌رسانی به برنامه مشتری/میزبان از به‌روزرسانی‌ها فراخوانی کند را بررسی می‌کند.

| ویژگی    | مورد استفاده                                                                                                                                        | پشتیبانی MCP                                                        |
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| دوام | سرور در طول وظیفه مهاجرت داده خراب می‌شود. نتایج و پیشرفت پس از راه‌اندازی مجدد باقی می‌مانند، مشتری می‌تواند وضعیت را بررسی کرده و از منبع پایدار ادامه دهد. | ✅ لینک‌های منبع با ذخیره‌سازی پایدار و اعلان‌های وضعیت |

امروز، یک الگوی رایج این است که ابزاری طراحی شود که یک منبع ایجاد کند و بلافاصله یک لینک منبع بازگشتی ارائه دهد. ابزار می‌تواند در پس‌زمینه به وظیفه رسیدگی کند، اعلان‌های منبعی که به‌روزرسانی‌های پیشرفت را ارائه می‌دهند یا نتایج جزئی را شامل می‌شوند صادر کند و محتوا را در منبع به‌روزرسانی کند.
...
**پشتیبانی MCP**: کاملاً از طریق نمونه‌گیری (برای ورودی هوش مصنوعی) و الیسیتیشن (برای ورودی انسانی) پشتیبانی می‌شود.

| ویژگی                 | مورد استفاده                                                                                                                                     | پشتیبانی MCP                                           |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------- |
| تعاملات چند نوبتی | عامل رزرو سفر تأیید قیمت را از کاربر درخواست می‌کند، سپس از هوش مصنوعی می‌خواهد داده‌های سفر را قبل از تکمیل تراکنش رزرو خلاصه کند. | ✅ الیسیتیشن برای ورودی انسانی، نمونه‌گیری برای ورودی هوش مصنوعی |

<div align="center" style="font-style: italic; font-size: 0.95em; margin-bottom: 0.5em;">
<strong>شکل 4:</strong> این نمودار نشان می‌دهد که چگونه عوامل MCP می‌توانند به طور تعاملی ورودی انسانی را درخواست کنند یا در میانه اجرا از هوش مصنوعی کمک بخواهند، و از جریان‌های کاری پیچیده و چند نوبتی مانند تأییدها و تصمیم‌گیری‌های پویا پشتیبانی کنند.
</div>
...
به عنوان بخشی از این مقاله، ما یک [مخزن کد](https://github.com/victordibia/ai-tutorials/tree/main/MCP%20Agents) ارائه می‌دهیم که شامل یک پیاده‌سازی کامل از عوامل طولانی‌مدت با استفاده از SDK پایتون MCP با انتقال StreamableHTTP برای از سرگیری جلسه و بازپخش پیام‌ها است. این پیاده‌سازی نشان می‌دهد که چگونه قابلیت‌های MCP می‌توانند ترکیب شوند تا رفتارهای پیچیده عامل‌گونه را فعال کنند.

به طور خاص، ما یک سرور با دو ابزار عامل اصلی پیاده‌سازی می‌کنیم:

- **عامل سفر** - یک سرویس رزرو سفر را با تأیید قیمت از طریق الیسیتیشن شبیه‌سازی می‌کند
- **عامل تحقیق** - وظایف تحقیقاتی را با خلاصه‌های کمک‌شده توسط هوش مصنوعی از طریق نمونه‌گیری انجام می‌دهد

هر دو عامل به‌روزرسانی‌های پیشرفت در زمان واقعی، تأییدهای تعاملی و قابلیت‌های کامل از سرگیری جلسه را نشان می‌دهند.
...
الیسیتیشن به عوامل امکان می‌دهد در میانه اجرا ورودی کاربر را درخواست کنند. این برای تأییدها، توضیحات یا تأییدها در طول وظایف طولانی‌مدت ضروری است.
...
**پیاده‌سازی مشتری (میزبان callback الیسیتیشن ارائه می‌دهد):**
...
**پیاده‌سازی سرور (عامل کمک هوش مصنوعی درخواست می‌کند):**
...
**پیاده‌سازی مشتری (میزبان callback نمونه‌گیری ارائه می‌دهد):**
...
#### قابلیت از سرگیری - تداوم جلسه در قطع ارتباط‌ها

قابلیت از سرگیری اطمینان می‌دهد که وظایف طولانی‌مدت عامل می‌توانند قطع ارتباط مشتری را تحمل کنند و پس از اتصال مجدد بدون وقفه ادامه یابند. این از طریق ذخیره رویدادها و توکن‌های از سرگیری پیاده‌سازی می‌شود.
...
**متاداده مشتری با توکن از سرگیری (مشتری با استفاده از وضعیت ذخیره‌شده دوباره متصل می‌شود):**
...
برنامه میزبان شناسه‌های جلسه و توکن‌های از سرگیری را به صورت محلی نگه می‌دارد و به آن امکان می‌دهد بدون از دست دادن پیشرفت یا وضعیت به جلسات موجود دوباره متصل شود.
...
- **`server/server.py`** - سرور MCP قابل از سرگیری با عوامل سفر و تحقیق که الیسیتیشن، نمونه‌گیری و به‌روزرسانی‌های پیشرفت را نشان می‌دهند
- **`client/client.py`** - برنامه میزبان تعاملی با پشتیبانی از سرگیری، مدیریت callback و مدیریت توکن
- **`server/event_store.py`** - پیاده‌سازی ذخیره رویداد که امکان از سرگیری جلسه و بازپخش پیام‌ها را فراهم می‌کند

## گسترش به ارتباط چندعاملی روی MCP
...
- **هماهنگی چند سرور**: میزبان اتصالات به چندین سرور MCP را که هر کدام قابلیت‌های عامل مختلفی را ارائه می‌دهند، حفظ می‌کند
- **مدیریت وضعیت وظیفه**: میزبان پیشرفت را در چندین وظیفه عامل همزمان ردیابی کرده و وابستگی‌ها و توالی‌ها را مدیریت می‌کند
- **انعطاف‌پذیری و تلاش مجدد**: میزبان خرابی‌ها را مدیریت کرده، منطق تلاش مجدد را پیاده‌سازی کرده و وظایف را در صورت عدم دسترسی عوامل مجدداً مسیردهی می‌کند
...
قابلیت‌های پیشرفته MCP - اعلان‌های منابع، الیسیتیشن/نمونه‌گیری، جریان‌های قابل از سرگیری و منابع پایدار - تعاملات پیچیده عامل-به-عامل را در عین حفظ سادگی پروتکل فعال می‌کنند.
...
- `clean-tokens` - پاک کردن توکن‌های از سرگیری
- `help` - نمایش راهنمای دستورات دقیق
- `quit` - خروج از مشتری

### 2. آزمایش قابلیت‌های از سرگیری

- یک عامل طولانی‌مدت را شروع کنید (مثلاً `travel_agent`)
- مشتری را در طول اجرا قطع کنید (Ctrl+C)
- مشتری
