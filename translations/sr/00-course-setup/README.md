...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-10-15T11:55:51+00:00",
...
### Плитко клонирање (препоручено за радионице / Codespaces)
  ...
  >Цео репозиторијум може бити велики (~3 GB) када преузимате целу историју и све датотеке. Ако само присуствујете радионици или вам требају само неке лекције, плитко клонирање (или делимично клонирање) избегава већину тог преузимања скраћивањем историје и/или прескакањем великих датотека.
...
#### Делимично (спарсно) клонирање — минималне датотеке + само изабране фасцикле

Ово користи делимично клонирање и спарсно-преглед (захтева Git 2.25+ и препоручује се модерни Git са подршком за делимично клонирање):
...
- У терминалу новоотвореног Codespace-а, покрените једну од команди за плитко/спарсно клонирање изнад да доведете само фасцикле лекција које су вам потребне у Codespace радни простор.
...
- Напомена: Ако више волите да директно отворите репозиторијум у Codespaces (без додатног клонирања), будите свесни да ће Codespaces конструисати devcontainer окружење и можда ће и даље обезбедити више него што вам је потребно. Клонирање плитке копије у свеж Codespace даје вам више контроле над употребом диска.

#### Савети

- Увек замените URL за клонирање са вашим форком ако желите да уређујете/комитујете.
- Ако касније будете требали више историје или датотека, можете их преузети или прилагодити спарсно-преглед да укључите додатне фасцикле.

## Покретање кода

Овај курс нуди серију Jupyter Notebooks-а које можете покренути да бисте стекли практично искуство у изградњи AI агената.
...
- Python 3.12+
  - **НАПОМЕНА**: Ако немате Python 3.12 инсталиран, уверите се да сте га инсталирали. Затим направите ваш venv користећи python3.12 да бисте осигурали да су исправне верзије инсталиране из датотеке requirements.txt.
  
    >Пример

    Направите фасциклу за Python venv:
...
```
Препоручујемо креирање Python виртуелног окружења да избегнете било какве конфликте и проблеме.
...
Ово можете урадити тако што ћете отићи на <a href="https://github.com/settings/personal-access-tokens" target="_blank">подешавања Personal Access Token-а</a> у вашем GitHub налогу.

Молимо вас да следите [Принцип најмање привилегије](https://docs.github.com/en/get-started/learning-to-code/storing-your-secrets-safely) приликом креирања вашег токена. То значи да треба да дате токену само оне дозволе које су му потребне за покретање узорака кода у овом курсу.
...
4. Ограничите дозволе токена: Под **Permissions**, кликните на таб **Account**, а затим на дугме "+ Add permissions". Појавиће се падајући мени. Потражите **Models** и означите поље за њега.
...
Ово ће копирати пример датотеке и направити `.env` у вашем директоријуму где ћете попунити вредности за променљиве окружења.
...
- `AZURE_OPENAI_SERVICE` - Пронађите ово у табу **Included capabilities** за **Azure OpenAI Service** на **Overview** страници.
