...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-21T12:59:54+00:00",
...
| 功能         | 使用場景                                                                                                                                                   | MCP 支持                                                                |
| ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------- |
| 可恢復性     | 客戶端在長時間運行的任務中斷線。重新連接後，會話恢復，重放錯過的事件，無縫繼續執行。                                                                      | ✅ StreamableHTTP 傳輸支持會話 ID、事件重放和 EventStore                 |
...
**MCP 支持**：MCP 現在支持工具調用返回資源鏈接類型。當前，一種可能的模式是設計一個工具，該工具創建一個資源並立即返回資源鏈接。工具可以在後台繼續處理任務並更新資源。客戶端則可以選擇輪詢該資源的狀態以獲取部分或完整結果（基於服務器提供的資源更新），或者訂閱該資源以接收更新通知。

這裡的一個限制是，輪詢資源或訂閱更新可能會消耗資源，並對規模產生影響。社區提案（包括 #992）正在探索服務器可以調用的 webhook 或觸發器，以通知客戶端/主應用程序更新的可能性。

| 功能         | 使用場景                                                                                                                                        | MCP 支持                                                        |
| ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| 持久性       | 服務器在數據遷移任務中崩潰。結果和進度在重啟後仍然存在，客戶端可以檢查狀態並從持久化資源繼續執行。                                              | ✅ 資源鏈接支持持久存儲和狀態通知                               |

當前，一種常見模式是設計一個工具，該工具創建一個資源並立即返回資源鏈接。工具可以在後台處理任務，發送資源通知作為進度更新或包含部分結果，並根據需要更新資源內容。

<div align="center" style="font-style: italic; font-size: 0.95em; margin-bottom: 0.5em;">
<strong>圖 3：</strong>此圖展示了 MCP 代理如何使用持久資源和狀態通知，確保長時間運行的任務在服務器重啟後仍然可用，允許客戶端檢查進度並檢索結果。
</div>
...
具體來說，我們實現了一個包含兩個主要代理工具的服務器：

- **旅遊代理** - 模擬一個通過引導進行價格確認的旅遊預訂服務
- **研究代理** - 通過採樣進行 AI 幫助總結的研究任務
...
- `research_agent` - 通過採樣進行 AI 幫助總結的研究
- `list` - 顯示所有可用工具
- `clean-tokens` - 清除恢復令牌
- `help` - 顯示詳細命令幫助
- `quit` - 退出客戶端
