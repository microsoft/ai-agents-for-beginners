...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-13T11:21:07+00:00",
  "source_file": "03-agentic-design-patterns/README.md",
  "language_code": "mo"
}
-->
[![如何設計優質 AI 代理](../../../translated_images/lesson-3-thumbnail.1092dd7a8f1074a5.mo.png)](https://youtu.be/m9lM8qqoOEA?si=4KimounNKvArQQ0K)
...
有許多方法可以思考如何構建 AI 代理系統。由於在生成式 AI 設計中，模糊性是一種特性而非缺陷，工程師有時很難知道從何開始。我們創建了一套以人為中心的 UX 設計原則，幫助開發者構建以客戶為中心的代理系統，以解決其業務需求。這些設計原則並非一種規範的架構，而是為定義和構建代理體驗的團隊提供的起點。
...
![代理設計原則](../../../translated_images/agentic-design-principles.1cfdf8b6d3cc73c2.mo.png)
...
- **過去**：反思包含狀態和上下文的歷史。
  ...
  - 代理從過去事件中創造連結，並積極反思記憶以應對當前情況。
- **現在**：推動而非通知。
  - 代理採用全面的方法與人互動。當事件發生時，代理超越靜態通知或其他形式的靜態互動。代理可以簡化流程或動態生成提示，在適當時刻引導用戶的注意力。
  - 代理根據上下文環境、社會和文化變化以及用戶意圖提供信息。
  - 代理互動可以是漸進的，隨著時間的推移逐步增長複雜性，以賦能用戶。
- **未來**：適應和演化。
  - 代理適應各種設備、平台和模式。
...
- **擁抱不確定性但建立信任**。
  - 代理設計中預期存在一定程度的不確定性。不確定性是代理設計的一個關鍵元素。
  - 信任和透明是代理設計的基礎層。
...
1. **透明性**：告知用戶 AI 的參與方式、運作方式（包括過去的行為），以及如何提供反饋和修改系統。
...
假設您正在設計一個旅行代理，以下是如何使用設計原則和指南的思路：

1. **透明性** – 告知用戶旅行代理是一個 AI 驅動的代理。提供一些基本指導，例如“你好”消息、示例提示。在產品頁面上清楚記錄這些信息。顯示用戶過去提出的提示列表。清楚說明如何提供反饋（例如，點讚或點踩、發送反饋按鈕等）。明確說明代理是否有使用或主題限制。
2. **控制** – 確保用戶清楚如何修改代理，例如系統提示。允許用戶選擇代理的詳細程度、寫作風格，以及代理不應談論的內容。允許用戶查看並刪除任何相關文件或數據、提示和過去的對話。
