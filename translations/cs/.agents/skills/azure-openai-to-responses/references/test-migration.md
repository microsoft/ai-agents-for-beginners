...
Při migraci kódu z Chat Completions na Responses API **testy selhávají předvídatelnými způsoby**. Tento odkaz popisuje, co opravit.
...
### Směrování mock odpovědí podle obsahu zprávy

Reálné aplikace vrací různé odpovědi na základě promptu. Směrujte podle `input` (nikoli `messages`):

```python
async def mock_acreate(*args, **kwargs):
    # Responses API používá 'input' místo 'messages'
...
| Předtím (mock Chat Completions) | Potom (mock Responses) |
|---------------------------------|-----------------------|
| `openai.types.chat.ChatCompletionChunk(...)` | `MockResponseEvent(event_type, delta)` |
| `choices[0].delta.content` | `event.delta` |
| `finish_reason="stop"` v chunku | `event.type == "response.completed"` |
| Azure-specifický chunk `prompt_filter_results` | Kompletně odstranit |
| Azure-specifické `content_filter_results` na volbu | Kompletně odstranit |
| `kwargs.get("messages")` v mocku | `kwargs.get("input")` v mocku |

---

## 2. Aktualizujte snapshot / zlaté soubory

Pokud testovací sada používá snapshot testování (např. `pytest-snapshot`, syrupy nebo vlastní JSONL snapshoty), změní se očekávaný tvar výstupu:

**Předtím** (streamovaný JSONL Chat Completions):
```jsonl
...
**Potom** (streamovaný JSONL Responses API):
```jsonl
{"delta": {"content": "The"}}
...
Nový tvar je výrazně jednodušší — neobsahuje pole `function_call`, `refusal`, `role`, `tool_calls`, `index`, `logprobs` ani `content_filter_results`. Aktualizujte nebo obnovte všechny snapshot soubory.

> **Tip**: Po migraci spusťte testy s `--snapshot-update` (pytest-snapshot) nebo `--update-snapshots` (syrupy) pro automatickou obnovu.

---

## 3. Aktualizujte testovací aserce

Běžné chyby asercí:

| Starý očekávaný výraz | Problém | Nový očekávaný výraz |
|----------------------|---------|----------------------|
...
| `client.api_version == "2024-..."` | Na `OpenAI`/`AsyncOpenAI` není `api_version` | Odstranit celé |
| `isinstance(client, AsyncAzureOpenAI)` | Typ klienta se změnil | `isinstance(client, AsyncOpenAI)` |

---

## 4. Aktualizujte environmentální proměnné v test fixture

Testy často nastavují env proměnné přes `monkeypatch.setenv`. Aktualizujte je takto:

| Starý env var | Nový env var | Poznámky |
|--------------|-------------|----------|
...
| `AZURE_OPENAI_CHAT_DEPLOYMENT` | `AZURE_OPENAI_CHAT_DEPLOYMENT` | Zachovat (název deploymentu pro parametr `model`) |
