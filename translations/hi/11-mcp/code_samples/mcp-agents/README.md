...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-24T09:36:04+00:00",
...
कार्यान्वयन में जाने से पहले, आइए उन बुनियादी ढांचा क्षमताओं को स्थापित करें जो लंबे समय तक चलने वाले एजेंटों का समर्थन करने के लिए आवश्यक हैं।
...
लंबे समय तक चलने वाले कार्यों के लिए पारंपरिक अनुरोध-प्रतिक्रिया पैटर्न काम नहीं करते। एजेंटों को प्रदान करने की आवश्यकता है:
...
| आंशिक परिणाम              | "एक किताब बनाएं" कार्य आंशिक परिणामों को स्ट्रीम करता है, जैसे 1) कहानी का आर्क आउटलाइन, 2) अध्याय सूची, 3) प्रत्येक अध्याय जैसे ही पूरा होता है। होस्ट किसी भी चरण में निरीक्षण, रद्द या पुनर्निर्देशित कर सकता है। | ✅ सूचनाएं "आंशिक परिणामों" को शामिल करने के लिए "विस्तारित" की जा सकती हैं, PR 383, 776 देखें |
...
**MCP समर्थन**: MCP StreamableHTTP transport आज सत्र पुनः शुरू करने और संदेश पुनः वितरण का समर्थन करता है session IDs और last event IDs के साथ। यहां महत्वपूर्ण बात यह है कि सर्वर को एक EventStore लागू करना चाहिए जो क्लाइंट पुनः कनेक्शन पर इवेंट रिप्ले को सक्षम बनाता है।  
ध्यान दें कि एक सामुदायिक प्रस्ताव (PR #975) है जो transport-agnostic resumable streams का पता लगाता है।

| सुविधा       | उपयोग का मामला                                                                                                                                                   | MCP समर्थन                                                                |
| ------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- |
| पुनः शुरू करने की क्षमता | लंबे समय तक चलने वाले कार्य के दौरान क्लाइंट डिस्कनेक्ट हो जाता है। पुनः कनेक्शन पर, सत्र फिर से शुरू होता है और छूटे हुए इवेंट्स को रिप्ले करता है, जहां से छोड़ा था वहां से निर्बाध रूप से जारी रहता है। | ✅ StreamableHTTP transport session IDs, इवेंट रिप्ले, और EventStore के साथ |

<div align="center" style="font-style: italic; font-size: 0.95em; margin-bottom: 0.5em;">
<strong>चित्र 2:</strong> यह आरेख दिखाता है कि MCP का StreamableHTTP transport और इवेंट स्टोर कैसे निर्बाध सत्र पुनः शुरू करने को सक्षम बनाता है: यदि क्लाइंट डिस्कनेक्ट हो जाता है, तो वह पुनः कनेक्ट कर सकता है और छूटे हुए इवेंट्स को रिप्ले कर सकता है, प्रगति की हानि के बिना कार्य जारी रख सकता है।
</div>
...
**MCP समर्थन**: MCP अब टूल कॉल के लिए Resource link return type का समर्थन करता है। आज, एक संभावित पैटर्न एक टूल को डिज़ाइन करना है जो एक resource बनाता है और तुरंत एक resource link लौटाता है। टूल पृष्ठभूमि में कार्य को संबोधित करना जारी रख सकता है और resource को अपडेट कर सकता है। बदले में, क्लाइंट इस resource की स्थिति की जांच करने के लिए चुन सकता है ताकि आंशिक या पूर्ण परिणाम प्राप्त किए जा सकें (आधारित resource अपडेट्स पर जो सर्वर प्रदान करता है) या resource के लिए अपडेट सूचनाओं की सदस्यता ले सकता है।
...
आज, एक सामान्य पैटर्न एक टूल को डिज़ाइन करना है जो एक resource बनाता है और तुरंत एक resource link लौटाता है। टूल पृष्ठभूमि में कार्य को संबोधित कर सकता है, resource सूचनाएं जारी कर सकता है जो प्रगति अपडेट के रूप में कार्य करती हैं या आंशिक परिणामों को शामिल करती हैं, और आवश्यकता के अनुसार resource की सामग्री को अपडेट कर सकता है।
...
इस लेख के हिस्से के रूप में, हम एक [कोड रिपॉजिटरी](https://github.com/victordibia/ai-tutorials/tree/main/MCP%20Agents) प्रदान करते हैं जिसमें MCP Python SDK का उपयोग करके लंबे समय तक चलने वाले एजेंटों का एक पूर्ण कार्यान्वयन शामिल है, जिसमें StreamableHTTP transport सत्र पुनः शुरू करने और संदेश पुनः वितरण के लिए है। कार्यान्वयन दिखाता है कि MCP क्षमताओं को कैसे संयोजित किया जा सकता है ताकि परिष्कृत एजेंट-जैसे व्यवहार सक्षम हो सकें।
...
दोनों एजेंट वास्तविक समय प्रगति अपडेट, इंटरैक्टिव पुष्टि, और पूर्ण सत्र पुनः शुरू करने की क्षमताओं का प्रदर्शन करते हैं।
...
पुनः शुरू करने की क्षमता सुनिश्चित करती है कि लंबे समय तक चलने वाले एजेंट कार्य क्लाइंट डिस्कनेक्शन को सहन कर सकते हैं और पुनः कनेक्शन पर निर्बाध रूप से जारी रह सकते हैं। इसे इवेंट स्टोर्स और पुनः शुरू करने वाले टोकन के माध्यम से लागू किया गया है।
...
**क्लाइंट मेटाडेटा पुनः शुरू करने वाले टोकन के साथ (क्लाइंट संग्रहीत स्थिति का उपयोग करके पुनः कनेक्ट करता है):**
...
होस्ट एप्लिकेशन स्थानीय रूप से session IDs और पुनः शुरू करने वाले टोकन बनाए रखता है, जिससे वह प्रगति या स्थिति खोए बिना मौजूदा सत्रों से पुनः कनेक्ट कर सकता है।
...
- **`server/server.py`** - यात्रा और अनुसंधान एजेंटों के साथ पुनः शुरू करने योग्य MCP सर्वर जो elicitation, sampling, और प्रगति अपडेट का प्रदर्शन करते हैं
- **`client/client.py`** - इंटरैक्टिव होस्ट एप्लिकेशन पुनः शुरू करने के समर्थन, callback हैंडलर्स, और टोकन प्रबंधन के साथ
- **`server/event_store.py`** - सत्र पुनः शुरू करने और संदेश पुनः वितरण को सक्षम करने वाला Event Store कार्यान्वयन

## MCP पर मल्टी-एजेंट संचार तक विस्तार

उपरोक्त कार्यान्वयन को होस्ट एप्लिकेशन की बुद्धिमत्ता और दायरे को बढ़ाकर मल्टी-एजेंट सिस्टम तक बढ़ाया जा सकता है:
