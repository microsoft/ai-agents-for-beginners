...
Ta primer prikazuje, kako zgraditi inteligentnega agenta za načrtovanje potovanj z uporabo Microsoft Agent Framework za .NET. Agent lahko samodejno generira personalizirane dnevne načrte za naključne destinacije po svetu.

### Ključne zmogljivosti:

- 🎲 **Naključen izbor destinacije**: Uporablja prilagojeno orodje za izbiro počitniških krajev
- 🗺️ **Inteligentno načrtovanje potovanj**: Ustvarja podrobne dnevne itinerarje
...
- **Azure OpenAI (Responses API)**: Uporablja Azure OpenAI Responses API za sklepanje modela
- **Azure Identity**: Varno prijavljanje prek `AzureCliCredential` (`az login`)
- **Varnostna konfiguracija**: Upravljanje končnih točk na podlagi okolja

### Ključne komponente

1. **AIAgent**: Glavni agent, ki ureja potek pogovora
2. **Prilagojena orodja**: Funkcija `GetRandomDestination()` na voljo agentu
3. **Responses Client**: Vmesnik za pogovor na osnovi Azure OpenAI Responses
4. **Podpora za pretakanje**: Zmožnosti za generiranje odzivov v realnem času
...
1. **Arhitektura agenta**: Microsoft Agent Framework nudi čist, varovan z vidika tipov pristop k gradnji AI agentov v .NET
2. **Integracija orodij**: Funkcije z atributi `[Description]` postanejo na voljo kot orodja za agenta
3. **Upravljanje konfiguracije**: Okoljske spremenljivke in varna obravnava poverilnic sledijo najboljšim praksam .NET
4. **Azure OpenAI Responses API**: Agent uporablja Azure OpenAI Responses API prek Azure.AI.OpenAI SDK
