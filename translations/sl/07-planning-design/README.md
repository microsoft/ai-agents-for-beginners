...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-30T09:25:06+00:00",
...
* Kako razdeliti kompleksno nalogo na obvladljive podnaloge in jih organizirati v logičnem zaporedju.
...
* Kako oceniti rezultate podnalog, meriti uspešnost in iterirati na dejanjih za izboljšanje končnega izhoda.

## Določitev splošnega cilja in razdelitev naloge
...
Večina nalog v resničnem svetu je preveč zapletenih, da bi jih lahko rešili v enem koraku. AI agent potrebuje jedrnat cilj, ki usmerja njegovo načrtovanje in dejanja. Na primer, razmislite o cilju:

    "Ustvari tridnevni potovalni načrt."

Čeprav je cilj preprost za zapisati, ga je treba še izpopolniti. Bolj kot je cilj jasen, bolj se lahko agent (in morebitni človeški sodelavci) osredotoči na dosego pravega rezultata, kot je ustvarjanje celovitega načrta z možnostmi letov, priporočili za hotele in predlogi za aktivnosti.
...
Vsako podnalogo lahko nato obravnavajo namenski agenti ali procesi. Eden od agentov se lahko specializira za iskanje najboljših ponudb za lete, drugi za rezervacije hotelov itd. Koordinacijski ali "nadaljnji" agent lahko nato združi te rezultate v enoten načrt za končnega uporabnika.
...
Veliki jezikovni modeli (LLM) lahko ustvarijo strukturiran izhod (npr. JSON), ki ga je lažje razčleniti in obdelati za nadaljnje agente ali storitve. To je še posebej uporabno v kontekstu več agentov, kjer lahko te naloge izvedemo po prejemu načrtovanega izhoda. Za hiter pregled si oglejte naslednji Python primer, ki prikazuje preprostega načrtovalnega agenta, ki razdeli cilj na podnaloge in ustvari strukturiran načrt:
...
* Prejme načrt za hotel: Načrtovalec vzame uporabniško sporočilo in na podlagi sistemskega poziva (vključno s podrobnostmi o razpoložljivih agentih) ustvari strukturiran potovalni načrt.
...
Naslednji Python primer kode ponazarja te korake:
...
Primer zvezka s prejšnjim primerom kode je na voljo [tukaj](07-autogen.ipynb).
...
Poleg tega lahko povratne informacije uporabnika (npr. če se človek odloči za zgodnejši let) sprožijo delno ponovno načrtovanje. Ta dinamičen, iterativen pristop zagotavlja, da se končna rešitev uskladi z resničnimi omejitvami in spreminjajočimi se uporabniškimi preferencami.
...
V tem članku smo si ogledali primer, kako lahko ustvarimo načrtovalca, ki lahko dinamično izbere razpoložljive agente, ki so definirani. Izhod načrtovalca razdeli naloge in dodeli agente, da jih lahko izvedejo. Predpostavlja se, da imajo agenti dostop do funkcij/orodij, ki so potrebna za izvedbo naloge. Poleg agentov lahko vključite tudi druge vzorce, kot so refleksija, povzemanje in krožni klepet, da dodatno prilagodite.

## Dodatni viri

* AutoGen Magnetic One - Generalistični sistem z več agenti za reševanje kompleksnih nalog, ki je dosegel impresivne rezultate na več zahtevnih agentnih merilih. Referenca:

. V tej implementaciji orkestrator ustvari načrt, specifičen za nalogo, in te naloge delegira razpoložljivim agentom. Poleg načrtovanja orkestrator uporablja tudi mehanizem za sledenje, da spremlja napredek naloge in po potrebi ponovno načrtuje.
...
[Vzorec oblikovanja z več agenti](../08-multi-agent/README.md)
