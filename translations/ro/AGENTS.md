...
4. **Set up environment variables:**
   ```bash
   cp .env.example .env
   # Editează fișierul .env cu cheile API și endpoint-urile tale
   ```
...
2. **Test notebook execution:**
   ```bash
   # Convertește notebook-ul în script și rulează-l (importuri pentru teste)
