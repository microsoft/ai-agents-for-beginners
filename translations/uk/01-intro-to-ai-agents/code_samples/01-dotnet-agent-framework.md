# 🌍 AI Тревел-агент з Microsoft Agent Framework (.NET)

## 📋 Огляд сценарію

Цей приклад демонструє, як створити інтелектуального агента для планування подорожей за допомогою Microsoft Agent Framework для .NET. Агент може автоматично генерувати персоналізовані маршрути одноденних подорожей у випадкові місця по всьому світу.

### Ключові можливості:

- 🎲 **Вибір випадкового напрямку**: Використовує власний інструмент для вибору місць відпочинку
- 🗺️ **Інтелектуальне планування подорожі**: Створює детальні маршрути день за днем
- 🔄 **Потокова трансляція в реальному часі**: Підтримує як миттєві, так і потокові відповіді
- 🛠️ **Інтеграція власних інструментів**: Демонструє, як розширити можливості агента
...
- **Azure OpenAI (Responses API)**: Використовує API відповідей Azure OpenAI для роботи моделей
- **Azure Identity**: Безпечний вхід через `AzureCliCredential` (`az login`)
- **Безпечне налаштування**: Керування кінцевими точками через змінні середовища

### Ключові компоненти

1. **AIAgent**: Головний оркестратор агента, який керує ходом розмови
...
4. **Підтримка потоків**: Можливості генерації відповідей у реальному часі

### Патерн інтеграції

```mermaid
graph LR
    A[Запит користувача] --> B[AI Агенція]
    B --> C[Azure OpenAI (API відповідей)]
    B --> D[Інструмент GetRandomDestination]
    C --> E[Маршрут подорожі]
    D --> E
```
...
# Потім увійдіть в систему, щоб AzureCliCredential міг отримати токен
az login
```
...
Або через командний рядок dotnet:
...
2. **Інтеграція інструментів**: Функції, позначені атрибутами `[Description]`, стають доступними інструментами агента
3. **Керування конфігурацією**: Змінні середовища та безпечне управління обліковими даними відповідно до кращих практик .NET
