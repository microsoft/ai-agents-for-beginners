...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-13T14:41:49+00:00",
  "source_file": "02-explore-agentic-frameworks/README.md",
  "language_code": "uk"
}
-->
[![Дослідження фреймворків AI агентів](../../../translated_images/uk/lesson-2-thumbnail.c65f44c93b8558df.png)](https://youtu.be/ODwF-EZo_O8?si=1xoy_B9RNQfrYdF7)
...
- Що таке фреймворки AI агентів і що вони дозволяють досягти розробникам?
...
Приклад: Віртуальні асистенти, такі як Siri та Google Assistant, використовують AI для розуміння та реагування на голосові команди, спрощуючи взаємодію користувачів із пристроями.
...
- **Використовуйте модульні компоненти**: SDK AI пропонують готові компоненти, такі як AI і Memory конектори, виклики функцій за допомогою природної мови або плагінів коду, шаблони підказок тощо.
...
- **Навчайтеся в реальному часі**: Реалізуйте зворотні зв'язки, де агенти навчаються від взаємодій і динамічно коригують свою поведінку.
...
<a href="https://en.wikipedia.org/wiki/Actor_model" target="_blank">Агенти базуються на акторній моделі</a>. Згідно з Вікіпедією, актор — це _основний будівельний блок паралельних обчислень. У відповідь на отримане повідомлення актор може: приймати локальні рішення, створювати більше акторів, надсилати більше повідомлень і визначати, як реагувати на наступне отримане повідомлення_.
...
- **Агенти**. Агент — це програмна сутність, яка:
  ...
  Ось короткий фрагмент коду, у якому створюється власний агент із можливостями чату:
    ...
    У попередньому коді створено `MyAgent`, який успадковується від `RoutedAgent`. Він має обробник повідомлень, який друкує вміст повідомлення, а потім надсилає відповідь за допомогою делегата `AssistantAgent`. Особливо зверніть увагу, як ми призначаємо `self._delegate` екземпляр `AssistantAgent`, який є попередньо створеним агентом, здатним обробляти завершення чату.
...
- **Середовище виконання агентів**. Фреймворк надає середовище виконання, яке забезпечує зв'язок між агентами, управляє їхніми ідентифікаторами та життєвими циклами, а також забезпечує дотримання меж безпеки та конфіденційності. Це означає, що ви можете запускати своїх агентів у безпечному та контрольованому середовищі, забезпечуючи їхню безпечну та ефективну взаємодію. Існує два середовища виконання, які заслуговують на увагу:
  - **Автономне середовище виконання**. Це хороший вибір для однопроцесних додатків, де всі агенти реалізовані однією мовою програмування та працюють в одному процесі. Ось ілюстрація того, як це працює:
...
- **AI конектори**: Це інтерфейс із зовнішніми AI сервісами та джерелами даних для використання як у Python, так і в C#.
  ...
  ```  
    ...
    Ось простий приклад того, як можна створ
...
- **Агент**. Сервіс Azure AI Agent інтегрується з Azure AI Foundry. У рамках AI Foundry, AI-агент діє як "розумний" мікросервіс, який може використовуватися для відповідей на запитання (RAG), виконання дій або повної автоматизації робочих процесів. Це досягається шляхом поєднання потужності генеративних AI моделей з інструментами, які дозволяють отримувати доступ до реальних джерел даних і взаємодіяти з ними. Ось приклад агента:
...
- **Потік і повідомлення**. Потік — це ще одна важлива концепція. Він представляє собою розмову або взаємодію між агентом і користувачем. Потоки можуть використовуватися для відстеження прогресу розмови, зберігання контекстної інформації та управління станом взаємодії. Ось приклад потоку:
...
- **Semantic Kernel**: Це готова до виробництва бібліотека агентів для створення корпоративних агентних застосувань. Зосереджується на подіях, розподілених агентних застосуваннях, дозволяючи використовувати кілька LLM і SLM, інструменти та одно-/мультиагентні шаблони дизайну.
...
[Вступ до AI-агентів і сфер застосування агентів](../01-intro-to-ai-agents/README.md)
