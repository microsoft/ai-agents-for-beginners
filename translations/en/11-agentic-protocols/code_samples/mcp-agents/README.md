...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-30T10:13:15+00:00",
...
> TL;DR - Can you build Agent-to-Agent Communication on MCP? Yes!
...
As developers increasingly explore tools with agent-like behaviors (e.g., running for extended periods, requiring additional input mid-execution), a common misconception is that MCP is unsuitable—primarily because early examples of its tools primitive focused on simple request-response patterns.

This perception is outdated. The MCP specification has been significantly enhanced in recent months, closing the gap for building long-running agentic behavior:
...
For clarity, we will refer to an agent as a "tool" available on an MCP server. This implies the presence of a host application that implements an MCP client, establishes a session with the MCP server, and interacts with the agent.
...
**MCP Support**: Resource update notifications enable streaming partial results, though careful design is required to avoid conflicts with JSON-RPC's 1:1 request/response model.

| Feature                    | Use Case                                                                                                                                                                       | MCP Support                                                                                |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------ |
| Real-time Progress Updates | A user requests a codebase migration task. The agent streams progress: "10% - Analyzing dependencies... 25% - Converting TypeScript files... 50% - Updating imports..."          | ✅ Progress notifications                                                                  |
| Partial Results            | A "Generate a book" task streams partial results, e.g., 1) Story arc outline, 2) Chapter list, 3) Each chapter as completed. The host can inspect, cancel, or redirect at any stage. | ✅ Notifications can be "extended" to include partial results (see proposals on PR 383, 776) |
...
| Feature      | Use Case                                                                                                                                                   | MCP Support                                                                |
| ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- |
| Resumability | A client disconnects during a long-running task. Upon reconnection, the session resumes with missed events replayed, continuing seamlessly from where it left off. | ✅ StreamableHTTP transport with session IDs, event replay, and EventStore |
...
**MCP Support**: MCP supports a Resource link return type for tool calls. A common pattern is to design a tool that creates a resource and immediately returns a resource link. The tool can continue working on the task in the background and update the resource. The client can poll the resource for partial or full results or subscribe to updates.
...
| Feature    | Use Case                                                                                                                                        | MCP Support                                                        |
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| Durability | A server crashes during a data migration task. Results and progress survive the restart, and the client can check status and continue from the persistent resource. | ✅ Resource links with persistent storage and status notifications |
...
| Feature                 | Use Case                                                                                                                                     | MCP Support                                           |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------- |
| Multi-Turn Interactions | A travel booking agent requests price confirmation from the user, then asks AI to summarize travel data before completing the booking transaction. | ✅ Elicitation for human input, sampling for AI input |
...
- **Intelligent Task Decomposition**: The host analyzes complex user requests and breaks them into subtasks for different specialized agents
- **Multi-Server Coordination**: The host maintains connections to multiple MCP servers, each exposing different agent capabilities
- **Task State Management**: The host tracks progress across multiple concurrent agent tasks, handling dependencies and sequencing
- **Resilience & Retries**: The host manages failures, implements retry logic, and reroutes tasks when agents become unavailable
- **Result Synthesis**: The host combines outputs from multiple agents into coherent final results
...
Ready to build your own agent-to-agent system? Follow these steps:
...
Overall, the MCP protocol spec is evolving rapidly; readers are encouraged to review the official documentation website for the most recent updates - https://modelcontextprotocol.io/introduction
