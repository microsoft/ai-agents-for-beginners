...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-10-15T11:35:54+00:00",
...
Dies kann durch Klicken auf den Link zum <a href="https://github.com/microsoft/ai-agents-for-beginners/fork" target="_blank">Fork des Repos</a> erfolgen.
...
Dies verwendet einen partiellen Clone und Sparse-Checkout (erfordert Git 2.25+ und wird mit moderner Git-Version mit Unterstützung für partielle Clones empfohlen):
...
Nachdem Sie die Dateien geklont und überprüft haben, können Sie, wenn Sie nur die Dateien benötigen und Speicherplatz freigeben möchten (keine Git-Historie), die Repository-Metadaten löschen (💀irreversibel — Sie verlieren alle Git-Funktionen: keine Commits, Pulls, Pushes oder Zugriff auf die Historie).
...
- Erstellen Sie einen neuen Codespace für dieses Repo über die [GitHub-Benutzeroberfläche](https://github.com/codespaces).  
...
- Optional: Entfernen Sie nach dem Klonen innerhalb von Codespaces `.git`, um zusätzlichen Speicherplatz zurückzugewinnen (siehe oben die Löschbefehle).
- Hinweis: Wenn Sie es vorziehen, das Repo direkt in Codespaces zu öffnen (ohne zusätzlichen Clone), beachten Sie, dass Codespaces die Devcontainer-Umgebung erstellen und möglicherweise mehr bereitstellen, als Sie benötigen. Das Klonen einer flachen Kopie in einem frischen Codespace gibt Ihnen mehr Kontrolle über die Festplattennutzung.

#### Tipps

- Ersetzen Sie immer die Clone-URL durch Ihren Fork, wenn Sie bearbeiten/committen möchten.
...
Dies kann durch Aufrufen Ihrer <a href="https://github.com/settings/personal-access-tokens" target="_blank">Einstellungen für persönliche Zugriffstoken</a> in Ihrem GitHub-Konto erfolgen.
...
2. Geben Sie einen beschreibenden Namen für Ihr Token ein, der seinen Zweck widerspiegelt, sodass es später leicht zu identifizieren ist.
...
4. Beschränken Sie die Berechtigungen des Tokens: Unter **Permissions** klicken Sie auf den Tab **Account** und dann auf die Schaltfläche "+ Add permissions". Es erscheint ein Dropdown-Menü. Suchen Sie nach **Models** und aktivieren Sie das Kästchen dafür.
...
Dies kann durch Aufrufen der **Übersichtsseite** Ihres Projekts im Azure AI Foundry-Portal erfolgen.
...
- `AZURE_OPENAI_SERVICE` - Finden Sie dies im Tab **Eingeschlossene Fähigkeiten** für **Azure OpenAI Service** auf der **Übersichtsseite**.
