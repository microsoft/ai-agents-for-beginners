...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-10-15T11:40:09+00:00",
...
在開始克隆您的倉庫之前，請加入 [AI Agents For Beginners Discord 頻道](https://aka.ms/ai-agents/discord)，以獲得設置幫助、課程相關問題的解答，或與其他學習者建立聯繫。
...
這使用部分克隆和稀疏檢出（需要 Git 2.25+，並建議使用支持部分克隆的現代 Git）：
...
- 在新創建的 Codespace 的終端中，運行上述淺克隆/稀疏克隆命令，僅將您需要的課程文件夾帶入 Codespace 工作空間。
...
- 如果您之後需要更多的歷史記錄或文件，可以抓取它們或調整稀疏檢出以包含其他文件夾。

## 運行代碼

本課程提供了一系列 Jupyter Notebook，您可以通過它們獲得動手體驗，學習如何構建 AI Agents。
...
您可以通過前往您的 <a href="https://github.com/settings/personal-access-tokens" target="_blank">個人訪問令牌設置</a> 在 GitHub 帳戶中創建令牌。

請在創建令牌時遵循 [最小特權原則](https://docs.github.com/en/get-started/learning-to-code/storing-your-secrets-safely)。這意味著您應僅授予令牌運行本課程代碼範例所需的權限。
...
3. 將令牌的範圍限制為此倉庫的分叉。
    ...
    ![限制範圍到分叉倉庫](../../../translated_images/token_repository_limit.924ade5e11d9d8bb6cd21293987e4579dea860e2ba66d607fb46e49524d53644.tw.png)

4. 限制令牌的權限：在 **Permissions** 下，點擊 **Account** 標籤，然後點擊 "+ Add permissions" 按鈕。將出現下拉菜單。請搜索 **Models** 並勾選它。
    ![添加模型權限](../../../translated_images/add_models_permissions.c0c44ed8b40fc143dc87792da9097d715b7de938354e8f771d65416ecc7816b8.tw.png)
...
6. 在生成令牌之前，請確保您準備好將令牌存儲在安全的地方，例如密碼管理器保險庫，因為生成後不會再次顯示。 ![安全存儲令牌](../../../translated_images/store_token_securely.08ee2274c6ad6caf3482f1cd1bad7ca3fdca1ce737bc485bfa6499c84297c789.tw.png)

複製您剛剛創建的新令牌。現在，您需要將此令牌添加到本課程包含的 `.env` 文件中。
...
您可以通過前往 Azure AI Foundry 入口網站中項目的 **概述** 頁面來完成此操作。

![項目連接字符串](../../../translated_images/project-endpoint.8cf04c9975bbfbf18f6447a599550edb052e52264fb7124d04a12e6175e330a5.tw.png)
...
作為安全最佳實踐，我們將使用 [無密鑰身份驗證](https://learn.microsoft.com/azure/developer/ai/keyless-connections?tabs=csharp%2Cazure-cli?WT.mc_id=academic-105485-koreyst) 來通過 Microsoft Entra ID 驗證到 Azure OpenAI。
...
- `AZURE_OPENAI_RESOURCE_GROUP` - 前往 **管理中心** 的 **概述** 頁面中的 **項目屬性**。
...
- `AZURE_OPENAI_ENDPOINT` - 查找 **Azure AI services**，點擊它，然後前往 **資源管理**，**密鑰和端點**，向下滾動到 "Azure OpenAI endpoints"，並複製顯示 "Language APIs" 的端點。

- `AZURE_OPENAI_API_KEY` - 從同一屏幕中複製 KEY 1 或 KEY 2。

- `AZURE_SEARCH_SERVICE_ENDPOINT` - 找到您的 **Azure AI Search** 資源，點擊它，然後查看 **概述**。

- `AZURE_SEARCH_API_KEY` - 然後前往 **設置**，然後 **密鑰**，複製主密鑰或次密鑰。
