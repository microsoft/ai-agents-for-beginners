...
2. **Use a production receipt library.** Several open-source projects implement the same pattern with additional features (key rotation, batch verification, JWK Set distribution, integration with policy engines):
   ...
   - The **[affixio](https://www.npmjs.com/package/affixio)** Node.js SDK (`npm i affixio`) signs host-side yes/no ACTION receipts with ML-DSA-65 (FIPS 204), keeps a hash-chained Merkle audit journal, and never sends PII off the customer host. Source: [AffixIO/SDK](https://github.com/AffixIO/SDK).
